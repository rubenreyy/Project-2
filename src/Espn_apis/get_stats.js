...
async function fetch_top_team_record(sport,league) {
    ...
    let team_record = await await fetch(`http://sports.core.api.espn.com/v2/sports/${sport}/leagues/${league}/seasons/2023/types/2/teams/${fetch_t_team.id}/record?lang=en&region=us`).then(response => {return response.json()}).then((data) => {return data.items})
    
    console.log(team_record[0].stats)
    ...
    let ties = team_record[0].stats.find((element) => element.name === "ties")
